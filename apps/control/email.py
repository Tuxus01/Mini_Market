...
class SendEmailOnlineThread(threading.Thread):
    ...
    def run(self):
        ...
        html_content = loader.render_to_string('base/online/SendEmailOnline.html', {'VENTA':ordenventa, 'DETALLE':detalle, 'CLIENTE':cliente, 'Tipo_Pago':pago, 'PROCESO':proceso})
        #msg.attach_alternative(html_content, "text/html")
        #msg.send()
...
#Envio del email al administrador reportando acceso no autorizado
class SendEmailOnlineChangeStatusThread(threading.Thread):
    def __init__(self, id, usuario,Tipo_Pago, Proceso ):
        threading.Thread.__init__(self)
        #self.adjunto = adjunto
        self.id = id
